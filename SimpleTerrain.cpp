...
void SimpleTerrain::ReGenBuffers()
{
	...
	for (int z = 0-(map_size/2), i = 0; z < 0+(map_size/2), i < map_size; z++, i++)
	{
		for (int x = 0-(map_size/2), j = 0; x < 0+(map_size/2), j < map_size; x++, j++)
		{
			float r = m_map[i][j];
			index = (map_size * i) + j;
			vertices[index].position = D3DXVECTOR3(x, z, r);
			vertices[index].tex_uv = D3DXVECTOR2(j * (1.0f / (map_size)), i * (1.0f / (map_size)));
		}
	}
...
bool SimpleTerrain::InitializeBuffers()
{
	...
	float aspectRadio = (float)window_width/(float)window_height * 1.55f;
	...
	for (int z = 0; z < map_size; z++)
	{
		...
		{
			//float d = 4000.0f / (window_width - 1.0f) * (1.0f / (1.0f - (float)j / window_width) - 1.0f);
			//XMFLOAT3 pos = XMFLOAT3(d, aspectRadio * d * (float)(i - (window_height - 1) / 2.0f) / window_height, 0);

			float r = m_map[x][z];
			index = (map_size * x) + z;
			vertices[index].position = D3DXVECTOR3(x, r, z);
			//vertices[index].position = D3DXVECTOR3(pos.x*2, pos.y*2, 0);
			//vertices[index].color = D3DXVECTOR4(1.0f, 1.0f, 1.0f, 1.0f);

			vertices[index].tex_uv = D3DXVECTOR2(x * (1.0f / (map_size)), z * (1.0f / (map_size)));
		}
	}
